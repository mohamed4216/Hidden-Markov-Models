...
def gen_segments(_store, _participants=None, _labels=None, _extend_front=0, _extend_back=0, _type=None, **_kwargs):
	...
	for k, seg in gen_seg_table(_store, _participants, _labels, _type):
		for start, end, length, label in seg.values:
			yield _store.select(k + '/samples', start=start - _extend_front, stop=start+length + _extend_back, **_kwargs)
...
def test():
	...
	try:
		features = ['RLAaccx', 'RLAaccy', 'RLAaccz','RLAgyrx', 'RLAgyry', 'RLAgyrz','RLAroll', 'RLApitch', 'RLAyaw','RLAmagx', 'RLAmagy', 'RLAmagz']
